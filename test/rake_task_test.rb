...
describe "single db" do
  ...
  %w[db actual_db_schema].each do |namespace|
    describe "#{namespace}:rollback_branches" do
      ...
      describe "with acronyms defined" do
        ...
        after do
          ActiveSupport::Inflector.inflections(:en) { |inflect| inflect.acronyms.delete("ts360") }
        end
    ...
    describe "#{namespace}:rollback_branches:manual" do
      it "rolls back the migrations in the reversed order" do
        utils.prepare_phantom_migrations
        ...
        utils.simulate_input("y") do
          Rake::Task["#{namespace}:rollback_branches:manual"].invoke
          Rake::Task["#{namespace}:rollback_branches:manual"].reenable
        end
        assert_equal %i[second first], TestingState.down
        assert_empty utils.migrated_files
      end
    ...
    describe "#{namespace}:phantom_migrations" do
      it "shows the list of phantom migrations" do
        ActualDbSchema::Git.stub(:current_branch, "fix-bug") do
          utils.prepare_phantom_migrations
          Rake::Task["#{namespace}:phantom_migrations"].invoke
          Rake::Task["#{namespace}:phantom_migrations"].reenable
          ...
          assert_match(
            %r{   up     20130906111512  fix-bug  tmp/migrated/20130906111512_second.rb}, TestingState.output
          )
        end
      end
    end
